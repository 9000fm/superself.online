...
export const MACRO_NAMES = [
  'TEXTURE', 'MOTION', 'GLOW', 'REACH',
  'BURST', 'TRAIL', 'GHOST', 'PULSE',
  'COLOR', 'DENSITY', 'SPEED', 'PUDDLE',
] as const;
...
const ENDPOINTS: Record<MacroName, MacroEndpoints> = {
  ...
  GLOW: {
    ...
    whiteBaseOpacity:   { low: 0.25, high: 0.95 },
...
export function computeConfigFromMacros(macros: MacroState): Partial<Config> {
  ...
  for (const macroName of MACRO_NAMES) {
    ...
    for (const [key, ep] of Object.entries(endpoints)) {
      ...
      // If multiple macros touch the same key, last write wins.
      // Currently each key appears in only one macro.
      result[key] = val;
    }
  }
